...
class WorkspaceStore:
    ...
    def update_fulltext_status(
        ...
        *,
        ...
        rebuilt = self.patch_workbench_items(project_id, [paper_id], changes)
        for item in rebuilt.get("items", []):
            if item.get("candidate_id") == paper_id:
                return item
        return {}
    ...
    def save_fulltext_statuses(self, project_id: str, payload: dict[str, Any]) -> None:
        ...
        for candidate_id, state in payload.items():
    ...
    def _build_candidate_links(
        ...
        source_url = self._clean_optional_url(record.url)
